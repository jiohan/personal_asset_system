...
- MVP 고정: 2단계(대/소)
  ...
  - 카테고리 타입은 `INCOME`/`EXPENSE`/`TRANSFER`를 유지할 수 있다.
  - 다만 MVP 거래 API에서 `TRANSFER`의 `categoryId`는 저장하지 않으며 항상 `null`이다.

  MVP 기본 카테고리(예시)
  - 수입: 급여, 부수입, 이자, 기타
  - 지출(고정비): 주거/통신, 보험, 구독료
  - 지출(변동비): 식비, 교통, 카페/간식, 쇼핑, 의료/건강, 경조사
  - 이체(라이브러리/예약 타입): 투자이동, 현금이동
    - 주의: `TRANSFER` 카테고리 타입은 향후 자금 이동 분류용으로만 유지한다. 현재 거래 저장 시 `categoryId`에는 연결되지 않는다.
...
### 8.3 목록 API 규칙(쿼리/응답)
- 날짜 범위: `from <= txDate <= to` (inclusive start + inclusive end)
  - 예: 2026-02 전체: `from=2026-02-01`, `to=2026-02-28`
- 공통 쿼리 파라미터(거래 목록 기준)
  - 기간: `from`, `to`
...
예)
```
GET /api/v1/transactions?from=2026-02-01&to=2026-02-28&type=EXPENSE&needsReview=true&page=0&size=50&sort=txDate,desc&q=스타벅스
```
...
- 기간 필터: `from <= txDate <= to` (inclusive start + inclusive end)
...
| 인박스 `inboxCount` | `count(*)` | `needsReview=true` | `deletedAt!=null` | 현재는 summary API 필드가 아니라 `GET /transactions?needsReview=true` 별도 조회로 처리 |
...
카테고리별 지출은 현재 구현 기준으로 "리포트 제외가 아닌 모든 지출"을 집계한다.

- 집계 대상: `type=EXPENSE` AND `excludeFromReports=false` AND `deletedAt IS NULL`
- 정렬: `sum(amount)` 내림차순
- `categoryId IS NULL`인 항목은 응답에서 `categoryId=null`, `categoryName="Uncategorized"`로 반환될 수 있다.
- `needsReview`는 현재 Top N 집계에서 별도 제외 조건이 아니다.
...
- 카테고리 사용
  - MVP 거래 API에서는 `TRANSFER.categoryId`를 받지 않는다. 저장값은 항상 `null`이다.
  - `TRANSFER` 타입 카테고리 자체는 향후 자금 이동 분류용 라이브러리 타입으로만 유지한다.
